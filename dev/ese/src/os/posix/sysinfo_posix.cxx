...
{
    if ( cwchBase ) base[0] = L'\0';
    if ( cwchExt ) ext[0] = L'\0';

    if ( !path ) return;

    size_t cwch = 0;
    while ( path[cwch] != L'\0' ) cwch++;
    ...
    const WCHAR* leaf = ( iLastSlash == cwch ) ? path : ( path + iLastSlash + 1 );

    size_t cwchLeaf = 0;
    while ( leaf[cwchLeaf] != L'\0' ) cwchLeaf++;
    ...
    size_t cwchStem = ( iLastDot == cwchLeaf ) ? cwchLeaf : iLastDot;
    if ( cwchBase )
    {
        size_t n = cwchStem < cwchBase - 1 ? cwchStem : cwchBase - 1;
        for ( size_t i = 0; i < n; ++i ) base[i] = leaf[i];
        base[n] = L'\0';
    }
    ...
    if ( cwchExt && iLastDot != cwchLeaf )
    {
        ...
        size_t n = cwchExtIn < cwchExt - 1 ? cwchExtIn : cwchExt - 1;
        for ( size_t i = 0; i < n; ++i ) ext[i] = leaf[iLastDot + i];
        ext[n] = L'\0';
    }
}
...
BOOL FUtilSystemRestrictIdleActivity()
{
    ...
    BOOL fLastResult = g_fRestrictIdleActivity ? fTrue : fFalse;
...
LOCAL BOOL FGetSystemVersion()
{
    ...
    while ( *p >= '0' && *p <= '9' ) { major = major * 10 + (DWORD)( *p - '0' ); p++; }
    if ( *p == '.' )
    {
        p++;
        while ( *p >= '0' && *p <= '9' ) { minor = minor * 10 + (DWORD)( *p - '0' ); p++; }
    }
    if ( *p == '.' )
    {
        p++;
        while ( *p >= '0' && *p <= '9' ) { micro = micro * 10 + (DWORD)( *p - '0' ); p++; }
    }
