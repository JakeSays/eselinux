...
int _wcsicmp( const wchar_t* s1, const wchar_t* s2 )
{
    while ( *s1 && *s2 )
    {
        const int c1 = ( *s1 < 0x80 ) ? tolower( *s1 ) : *s1;
        const int c2 = ( *s2 < 0x80 ) ? tolower( *s2 ) : *s2;
...
int _wcsnicmp( const wchar_t* s1, const wchar_t* s2, size_t n )
{
    ...
    {
        const int c1 = ( s1[i] < 0x80 ) ? tolower( s1[i] ) : s1[i];
        const int c2 = ( s2[i] < 0x80 ) ? tolower( s2[i] ) : s2[i];
        if ( c1 != c2 ) return c1 - c2;
        if ( c1 == 0 ) return 0;
    }
    return 0;
...
LONG _wtol( const wchar_t* s )
{
    ...
    {
        acc = acc * 10 + ( *s - L'0' );
        if ( acc > 0x7fffffffLL ) return sign > 0 ? 0x7fffffffL : -0x7fffffffL - 1;
        ++s;
    }
    acc *= sign;
...
{
    if ( !wsz ) { if ( pwszEnd ) *pwszEnd = nullptr; return 0; }
    const wchar_t* p = wsz;
    while ( *p == L' ' || *p == L'\t' || *p == L'\n' || *p == L'\r' ) ++p;
    if ( *p == L'+' ) ++p;
    if ( radix == 0 ) radix = 10;
    if ( radix == 16 && p[0] == L'0' && ( p[1] == L'x' || p[1] == L'X' ) ) p += 2;
    ...
    if ( pwszEnd ) *pwszEnd = const_cast<wchar_t*>( p );
    return acc;
}
...
long long _wcstoi64( const wchar_t* wsz, wchar_t** pwszEnd, int radix )
{
    ...
    return sign < 0 ? -static_cast<long long>( mag ) : static_cast<long long>( mag );
}
...
{
    if ( !sz ) { if ( pszEnd ) *pszEnd = nullptr; return 0; }
...
static bool WideToNarrowAscii( const wchar_t* src, char* dst, size_t cch )
{
    ...
    {
        if ( static_cast<unsigned>( src[i] ) > 0x7f ) return false;
...
// Translate MSVC scanf size specifiers (%I64, %I32) into glibc equivalents.
static void TranslateMsvcFormat( char* fmt )
{
    char* p = fmt;
    while ( *p )
    {
...
// Minimal swscanf_s for 16-bit-WCHAR ASCII input/format. Engine usage scans
// numeric tokens only (no %s / %c), so we ignore the implicit buffer-size
// arguments the _s form would normally require. Returns the count of fields
// matched, like the underlying vsscanf.
int swscanf_s( const wchar_t* wsz, const wchar_t* wszFmt, ... )
{
    ...
    TranslateMsvcFormat( szFmt );
    va_list ap;
    va_start( ap, wszFmt );
    int r = vsscanf( szInput, szFmt, ap );
    va_end( ap );
    return r;
}
...
{
    if ( !buffer || cb == 0 ) return EINVAL;
    if ( radix != 8 && radix != 10 && radix != 16 ) { buffer[0] = 0; return EINVAL; }
    int n = 0;
    ...
    if ( n < 0 || static_cast<size_t>( n ) >= cb ) { buffer[0] = 0; return ERANGE; }
    return 0;
}
...
{
    if ( !ppf ) return EINVAL;
    *ppf = nullptr;
    char szPath[ 4096 ];
    char szMode[ 32 ];
    if ( !WideToNarrowAscii( wszFile, szPath, sizeof( szPath ) ) ) return EINVAL;
    if ( !WideToNarrowAscii( wszMode, szMode, sizeof( szMode ) ) ) return EINVAL;
...
{
    if ( !dst || cch == 0 ) return EINVAL;
    const size_t need = ( end >= begin ) ? static_cast<size_t>( end - begin ) : 0;
    if ( need + 1 > cch ) { dst[0] = 0; return ERANGE; }
    for ( size_t i = 0; i < need; ++i ) dst[i] = begin[i];
    dst[ need ] = 0;
    return 0;
}
...
{
    if ( !wszPath ) return EINVAL;
    if ( wszDrive && cchDrive ) wszDrive[0] = 0;
    const wchar_t* end = wszPath + WideLen( wszPath );
    const wchar_t* slash = end;
    while ( slash > wszPath && slash[-1] != L'/' && slash[-1] != L'\\' ) --slash;
    const wchar_t* dot = end;
    for ( const wchar_t* p = end; p > slash; --p ) { if ( p[-1] == L'.' ) { dot = p - 1; break; } }
    if ( dot < slash ) dot = end;
    int r = 0, t;
    if ( wszDir   && cchDir   ) { t = CopyWideRange( wszDir,   cchDir,   wszPath, slash ); if ( t ) r = t; }
    if ( wszFname && cchFname ) { t = CopyWideRange( wszFname, cchFname, slash,   dot );   if ( t ) r = t; }
    if ( wszExt   && cchExt   ) { t = CopyWideRange( wszExt,   cchExt,   dot,     end );   if ( t ) r = t; }
    return r;
}
...
{
    if ( !wszPath || cchPath == 0 ) return EINVAL;
    size_t n = 0;
    wszPath[0] = 0;
    ...
    {
        if ( n + 1 < cchPath ) { wszPath[ n++ ] = L'/'; wszPath[ n ] = 0; }
        else return ERANGE;
    }
...
// Wide-printf bridge. Engine wchar_t is 16-bit (-fshort-wchar); libc's
// wprintf/fwprintf operate on 32-bit native wchar_t and would print
// garbage. We route through StringCbVPrintfW (already wired up to handle
// 16-bit format strings + Win32 specifiers) into a stack buffer, then
// down-convert to UTF-8 via WideCharToMultiByte and write via fwrite.
extern "C" HRESULT StringCbVPrintfW( wchar_t* dst, size_t cbDst, const wchar_t* fmt, va_list args );

namespace
{
    int VFwprintfImpl( FILE* fp, const wchar_t* fmt, va_list args )
    {
        wchar_t wbuf[ 4096 ];
        StringCbVPrintfW( wbuf, sizeof( wbuf ), fmt, args );
...
int wprintf( const wchar_t* fmt, ... )
{
    ...
    const int n = VFwprintfImpl( stdout, fmt, a );
    va_end( a );
    return n;
}
...
{
    va_list a; va_start( a, fmt );
    const int n = VFwprintfImpl( fp, fmt, a );
    va_end( a );
    return n;
}
...
{
    return VFwprintfImpl( stdout, fmt, args );
}
