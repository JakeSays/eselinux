...
namespace
{
    ...
    bool FillFindData( const char* dir, const char* name, LPWIN32_FIND_DATAW out )
    {
        ...
        if ( ( st.st_mode & S_IWUSR ) == 0 ) out->dwFileAttributes |= FILE_ATTRIBUTE_READONLY;
        ...
        if ( n <= 0 ) return false;
        for ( int i = 0; i <= n && i < 260; ++i ) out->cFileName[ i ] = wbuf[ i ];
    ...
    void SplitPath( const char* full, char* dir, size_t dirSz, char* base, size_t baseSz )
    {
        ...
        else
        {
            ...
            const size_t cp = cb < dirSz - 1 ? cb : dirSz - 1;
            memcpy( dir, full, cp );
            dir[ cp ] = 0;
...
HANDLE FindFirstFileW( LPCWSTR lpFileName, LPWIN32_FIND_DATAW lpFindFileData )
{
    ...
    k->findPattern = strdup( base[ 0 ] ? base : "*" );
    ...
    closedir( d );
    k->findDir = nullptr;
    free( k->findBaseDir ); k->findBaseDir = nullptr;
    free( k->findPattern ); k->findPattern = nullptr;
    free( k );
    SetLastError( ERROR_FILE_NOT_FOUND );
    return INVALID_HANDLE_VALUE;
}
...
{
    if ( hFindFile == INVALID_HANDLE_VALUE || !hFindFile ) return TRUE;
