...
{
    char path[ c_pathBuf ];
    if ( WidePathToUtf8( lpFileName, path, sizeof( path ) ) <= 0 ) return FALSE;
    if ( unlink( path ) < 0 )
    {
...
BOOL MoveFileW( LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName )
{
    ...
    if ( WidePathToUtf8( lpExistingFileName, from, sizeof( from ) ) <= 0 ) return FALSE;
    if ( WidePathToUtf8( lpNewFileName,      to,   sizeof( to ) )   <= 0 ) return FALSE;
    if ( rename( from, to ) < 0 )
    {
...
BOOL CopyFileW( LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName, BOOL bFailIfExists )
{
    ...
    const int dstFlags = O_WRONLY | O_CREAT | ( bFailIfExists ? O_EXCL : O_TRUNC ) | O_CLOEXEC;
...
{
    char path[ c_pathBuf ];
    if ( WidePathToUtf8( lpFileName, path, sizeof( path ) ) <= 0 ) return INVALID_FILE_ATTRIBUTES;
    struct stat st;
    if ( stat( path, &st ) < 0 )
    {
        SetLastError( errno == ENOENT ? ERROR_FILE_NOT_FOUND : ERROR_ACCESS_DENIED );
...
BOOL SetFileAttributesW( LPCWSTR lpFileName, DWORD dwFileAttributes )
{
    ...
    mode_t m = st.st_mode;
    if ( dwFileAttributes & FILE_ATTRIBUTE_READONLY ) m &= ~( S_IWUSR | S_IWGRP | S_IWOTH );
    else                                              m |= S_IWUSR;
    return ( chmod( path, m ) == 0 ) ? TRUE : FALSE;
}

BOOL CreateDirectoryW( LPCWSTR lpPathName, LPSECURITY_ATTRIBUTES /*lpSecurityAttributes*/ )
{
    char path[ c_pathBuf ];
    if ( WidePathToUtf8( lpPathName, path, sizeof( path ) ) <= 0 ) return FALSE;
...
{
    char path[ c_pathBuf ];
    if ( WidePathToUtf8( lpFileName, path, sizeof( path ) ) <= 0 ) return 0;
    ...
    for ( int i = 0; i <= n; ++i ) lpBuffer[ i ] = wbuf[ i ];
...
UINT GetTempFileNameW( LPCWSTR lpPathName, LPCWSTR lpPrefixString, UINT uUnique, LPWSTR lpTempFileName )
{
    ...
    if ( WidePathToUtf8( lpPathName, dir, sizeof( dir ) ) <= 0 ) return 0;
    if ( WidePathToUtf8( lpPrefixString, prefix, sizeof( prefix ) ) <= 0 )
    {
        prefix[ 0 ] = 't'; prefix[ 1 ] = 0;
    }
    char path[ c_pathBuf ];
    UINT unique = uUnique != 0 ? uUnique : static_cast<UINT>( getpid() );
    ...
    if ( n <= 0 ) return 0;
    for ( int i = 0; i <= n; ++i ) lpTempFileName[ i ] = wbuf[ i ];
    return unique;
}
...
DWORD GetFinalPathNameByHandleW( HANDLE hFile, LPWSTR lpszFilePath, DWORD cchFilePath, DWORD /*dwFlags*/ )
{
    ...
    for ( int i = 0; i <= wn; ++i ) lpszFilePath[ i ] = wbuf[ i ];
