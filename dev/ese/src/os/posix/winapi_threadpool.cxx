...
void SetThreadpoolTimer(PTP_TIMER pti, PFILETIME pftDueTime, DWORD /*msPeriod*/, DWORD /*msWindowLength*/)
{
    ...
    else
    {
        ...
        if (t->fireAt.tv_nsec < 0)
        {
            t->fireAt.tv_sec -= 1;
            t->fireAt.tv_nsec += 1000000000;
        }
        if (t->fireAt.tv_nsec >= 1000000000)
        {
            t->fireAt.tv_sec += 1;
            t->fireAt.tv_nsec -= 1000000000;
        }
...
void SetThreadpoolWait(PTP_WAIT pwa, HANDLE h, PFILETIME pftTimeout)
{
    ...
    if (pftTimeout)
    {
        ...
        const uint64_t ms = (ft < 0)
                            ? static_cast<uint64_t>(-ft) / 10000
                            : 0;
        w->timeoutAt = now;
