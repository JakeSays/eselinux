...
{
    char path[ c_pathBuf ];
    if ( !lpRootPathName ) return DRIVE_FIXED;
    if ( WidePathToUtf8( lpRootPathName, path, sizeof( path ) ) <= 0 ) return DRIVE_UNKNOWN;
    struct stat st;
    if ( stat( path, &st ) < 0 ) return DRIVE_NO_ROOT_DIR;
    return DRIVE_FIXED;
}
                            ...
                            LPWSTR lpFileSystemNameBuffer, DWORD nFileSystemNameSize )
{
    ...
    if ( lpVolumeNameBuffer && nVolumeNameSize > 0 )
    {
        ...
        const DWORD cp = static_cast<DWORD>( n ) < nVolumeNameSize - 1
                            ? static_cast<DWORD>( n ) : nVolumeNameSize - 1;
        for ( DWORD i = 0; i < cp; ++i ) lpVolumeNameBuffer[ i ] = wbuf[ i ];
        lpVolumeNameBuffer[ cp ] = 0;
    }
    if ( lpVolumeSerialNumber )       *lpVolumeSerialNumber       = static_cast<DWORD>( st.st_dev );
    if ( lpMaximumComponentLength )   *lpMaximumComponentLength   = static_cast<DWORD>( vfs.f_namemax );
    ...
    if ( lpFileSystemNameBuffer && nFileSystemNameSize > 0 )
    {
        ...
        const DWORD cp = static_cast<DWORD>( n ) < nFileSystemNameSize - 1
                            ? static_cast<DWORD>( n ) : nFileSystemNameSize - 1;
        for ( DWORD i = 0; i < cp; ++i ) lpFileSystemNameBuffer[ i ] = wbuf[ i ];
...
BOOL GetVolumePathNameW( LPCWSTR lpszFileName, LPWSTR lpszVolumePathName, DWORD cchBufferLength )
{
    ...
    if ( !lpszVolumePathName || cchBufferLength < 2 ) return FALSE;
    ( void )lpszFileName;
...
{
    if ( !lpszVolumeName || cchBufferLength == 0 ) return FALSE;
                        ...
                        LPDWORD lpTotalNumberOfClusters )
{
    ...
    if ( lpBytesPerSector )         *lpBytesPerSector        = static_cast<DWORD>( vfs.f_bsize );
    if ( lpNumberOfFreeClusters )   *lpNumberOfFreeClusters  = static_cast<DWORD>( vfs.f_bavail );
    if ( lpTotalNumberOfClusters )  *lpTotalNumberOfClusters = static_cast<DWORD>( vfs.f_blocks );
    return TRUE;
}
                          ...
                          PULARGE_INTEGER lpTotalNumberOfBytes, PULARGE_INTEGER lpTotalNumberOfFreeBytes )
{
    ...
    if ( lpFreeBytesAvailable )      lpFreeBytesAvailable->QuadPart      = bs * vfs.f_bavail;
    if ( lpTotalNumberOfBytes )      lpTotalNumberOfBytes->QuadPart      = bs * vfs.f_blocks;
    if ( lpTotalNumberOfFreeBytes )  lpTotalNumberOfFreeBytes->QuadPart  = bs * vfs.f_bfree;
    return TRUE;
}
...
{
    if ( hFindVolume == INVALID_HANDLE_VALUE || !hFindVolume ) return TRUE;
                                       ...
                                       DWORD cchBufferLength, PDWORD lpcchReturnLength )
{
    ...
    {
        if ( lpcchReturnLength ) *lpcchReturnLength = 3;
