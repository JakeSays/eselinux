...
DWORD OSMemoryPageEvictionCount()
{
    ...
    if (tok)
        val = (DWORD) strtoul(tok, nullptr, 10);
    return val;
}
...
BOOL FOSMemoryPageCommit(void* const pv, const size_t cb)
{
    ...
    return mprotect(pvAligned, cbAligned, PROT_READ | PROT_WRITE) == 0
           ? fTrue
           : fFalse;
}
...
void OSMemoryPageReset(void* const pv, const size_t cbSize, const BOOL fToss)
{
    ...
    madvise(pvAligned, cbAligned, fToss
                                  ? MADV_DONTNEED
                                  : MADV_FREE);
}
...
BOOL FOSMemoryPageAllocated(const void* const pv, const size_t cb)
{
    ...
    if (cPage > sizeof(rgvec))
    {
        ...
        return mincore((void*) uPvAligned, cbPage, rgvec) == 0
               ? fTrue
               : fFalse;
    }
    return mincore((void*) uPvAligned, cbAligned, rgvec) == 0
           ? fTrue
           : fFalse;
}
...
void* PvOSMemoryHeapAllocAlign_(const size_t cbSize, const size_t cbAlign, __in_z const CHAR* /*szFile*/,
    LONG /*lLine*/)
{
    return PvOSMemoryHeapAllocAlign__(cbSize, cbAlign);
}

void* PvOSMemoryPageAlloc_(const size_t cbSize, void* const pvHint, const BOOL fAllocTopDown,
    __in_z const CHAR* /*szFile*/, LONG /*lLine*/)
{
...
COSMemoryMap::ERR COSMemoryMap::ErrOSMMPatternAlloc_(const size_t cbPattern,
    const size_t cbSize,
...
{
    if (cMap == 0 || cbMap == 0)
        return COSMemoryMap::ERR::errOutOfAddressSpace;
...
{
    if (!m_pvMap || cbCommit > m_cbMap)
        return fFalse;
    if (mprotect(m_pvMap, cbCommit, PROT_READ | PROT_WRITE) != 0)
        return fFalse;
    m_cbCommit = cbCommit;
    return fTrue;
}
...
COSMemoryMap::ERR COSMemoryMap::ErrOSMMPatternAlloc__(
    ...
    void** const ppvPattern)
{
    if (cbPattern == 0 || cbSize < cbPattern)
        return COSMemoryMap::ERR::errOutOfMemory;
