...
enum class PsiMode
{
    NotAvailable,   //  /proc/pressure/memory missing entirely — no poller, no signal
    Trigger,        //  Kernel accepted our trigger — poll(POLLPRI) on psiFd
    CounterPoll,    //  Trigger rejected (Ubuntu et al.) — poll counters every 1 s
};
...
void * PollerThreadMain( void * arg )
{
    ...
    const int timeoutMs    = fUseTrigger ? -1 : c_counterPollIntervalMs;
...
bool StartPsiPoller( PosixMemoryNotification * pn )
{
    ...
    else
    {
        ...
        pn->mode = ( w >= 0 ) ? PsiMode::Trigger : PsiMode::CounterPoll;
    }
                                  ...
                                  _Out_ BOOL * const          pfLowMemory )
{
    ...
    *pfLowMemory = fFalse;
    ...
    if ( psiAvg10 >= 0.0 )
    {
        *pfLowMemory = ( psiAvg10 >= c_psiQueryThresholdPercent ) ? fTrue : fFalse;
    }
