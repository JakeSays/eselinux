...
// shape by carrying the file's fd in a FileMappingObject and letting
// MapViewOfFile* call mmap() against it.
...
using osposix::As;
using osposix::FileMappingObject;
    ...
    LPCWSTR lpName)
{
    Unused(lpAttributes);
    Unused(lpName);
    FileObject* const file = As<FileObject>(hFile);
    if (!file || file->Fd() < 0)
    {
    ...
    const QWORD maxSize = ((QWORD)dwMaximumSizeHigh << 32) | (QWORD)dwMaximumSizeLow;
    return KToHandle(new FileMappingObject(fdDup, flProtect, maxSize));
}
...
{
    FileMappingObject* const k = As<FileMappingObject>(hFileMappingObject);
    if (!k || k->Fd() < 0)
    {
        SetLastError(ERROR_INVALID_HANDLE);
        return NULL;
    ...
    void* const pv = mmap(lpBaseAddress, cbView, prot, flags, k->Fd(), ibOffset);
    if (pv == MAP_FAILED)
    {
...
BOOL FlushViewOfFile(LPCVOID lpBaseAddress, SIZE_T dwNumberOfBytesToFlush)
{
    ...
    const size_t cb = dwNumberOfBytesToFlush != 0
