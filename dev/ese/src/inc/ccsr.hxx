...
{
public:
    using FixedArray<CSR, TAlloc>::FixedArray; // inherit base constructor

    // Copy and move constructors can't be inherited.
    CSRArray() = default;
    CSRArray( const FixedArray<CSR, TAlloc>& rhs ) : FixedArray<CSR, TAlloc>( rhs ) {}
    CSRArray( CSRArray&& rhs ) : FixedArray<CSR, TAlloc>( std::move( rhs ) ) {}
    CSRArray( FixedArray<CSR, TAlloc>&& rhs ) : FixedArray<CSR, TAlloc>( std::move( rhs ) ) {}  // both are needed because a base rvalue-reference can't bind to a derived type

    const CSRArray& operator=( CSRArray&& rhs ) { return static_cast<const CSRArray&>( FixedArray<CSR, TAlloc>::operator=( std::move( rhs ) ) ); }
    const CSRArray& operator=( FixedArray<CSR, TAlloc>&& rhs ) { return static_cast<const CSRArray&>( FixedArray<CSR, TAlloc>::operator=( std::move( rhs ) ) ); }
    ~CSRArray()
    {
        if ( m_fOwnsArray )
        {
