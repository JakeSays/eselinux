...
#include "platform.h"   // not_PubEsent: ESE_ARCH_64BIT + LP64 _JET_BASE_TYPES_DEFINED override (port)
...
// not_PubEsent: Linux platform lifecycle entry points (no Windows DllMain)
#ifndef _WIN32

//  Call JetPlatformInitialize() once per process before any other Jet API.
//  It does the work Windows hides inside libese.dll's DllMain plus the
//  Linux-specific bits: registers the user TLS size, suppresses the perfmon
//  path (not compiled into osposix), and bumps OSU's init counter so the
//  resource managers freeze with the right param defaults.  Idempotent —
//  extra calls return JET_errSuccess.  Pair with JetPlatformTerminate() at
//  process shutdown for deterministic teardown; otherwise the .so destructor
//  handles it.
JET_ERR JET_API JetPlatformInitialize( void );

//  Variant that pins the per-binary config file the engine merges on top of
//  /etc/ese.conf at startup.  Pass an absolute path; /etc/ese.conf (if
//  present) is always loaded first.  An empty / null szConfigPath is
//  equivalent to JetPlatformInitialize.
