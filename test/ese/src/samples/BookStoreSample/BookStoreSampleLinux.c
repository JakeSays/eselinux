...
// BookStoreSampleLinux.c : Linux port of BookStoreSample.c.
//
// The original BookStoreSample.c is the Windows reference and is kept
// untouched so it still tracks upstream. This file is the Linux variant:
// POSIX path separators for JET_param*Path, plain `int main` (no
// __cdecl), and an event-source name that doesn't pretend to be a .exe.
//
// We still include <windows.h> — on Linux that resolves to
// windows-shim/windows.h, which provides the NLS macros (MAKELCID,
// SORT_DEFAULT, LANG_ENGLISH...) the sample uses to build LCIDs for
// secondary indexes. ESE's Linux norm layer translates those LCIDs to
// ICU locales internally, so the LCID values themselves aren't load-
// bearing — they're just tags.
...
int main(int argc, char ** argv)
{
    ...
    Call( JetSetSystemParameter( &instance, 0, JET_paramEventSource, 0, "BookStoreSample" ) );
