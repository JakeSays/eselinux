...
typedef enum
{
    ...
    // No longer used. Left in to preserve the subsequent enum values.
...
typedef struct tagDBUTIL_A
{
    ...
    union
    {
        ...
        struct
        {
            ...
            uint32_t pgnoLast;
            // Last page to dump the category for. The last page in the database can be passed in as (ulong)-1.
            void* pfnSpaceCatCallback; // Callback to receive each page's category (JET_SPCATCALLBACK).
            void* pvContext;
            // General purpose context which is passed back to the client callback (pfnSpaceCatCallback).
        } spcatOptions;
...
typedef enum
{
    ...
    // Not a real flag, just a constant to signal no flags.

    // Expected/consistent categories.
    spcatfStrictlyLeaf = 0x00000001,
    // Page is strictly a leaf (i.e., not a root).
    spcatfStrictlyInternal = 0x00000002,
    // Page is strictly an internal page (i.e., not a root).
    spcatfRoot = 0x00000004,
    // Page is a root.
    spcatfSplitBuffer = 0x00000008,
    // Page is part of a split buffer.
    spcatfSmallSpace = 0x00000010,
    // Page is part of a small space tree.
    spcatfSpaceOE = 0x00000020,
    // Page belongs to an OE tree.
    spcatfSpaceAE = 0x00000040,
    // Page belongs to an AE tree (to the space tree itself, not a free/available page).
    spcatfAvailable = 0x00000080,
    // Page is free/available at the root or object level.
    ...
    // Page type could not be determined because full category search is not enabled.
    spcatfInconsistent = 0x08000000,
    // Page was found to be inconsistent during lookup.
    spcatfLeaked = 0x10000000,
    // Page is leaked at the root or object level.
    spcatfNotOwned = 0x20000000,
    // Page is not owned by the DB root and is before or at the last known-owned page (this means root OE corruption).
    spcatfNotOwnedEof = 0x40000000,
    // Page is not owned by the DB root and is beyond the last known-owned page.
...
typedef void (JET_API *JET_SPCATCALLBACK)(const uint32_t pgno, const uint32_t objid, const SpaceCategoryFlags spcatf,
...
typedef enum
{
    ...
    // [Informational] Information trace tag.
    JET_tracetagErrors,
    // [Informational] Errors trace tag.
    JET_tracetagAsserts,
    // [Error] Asserts trace tag.
    JET_tracetagAPI,
    JET_tracetagInitTerm,
    // High level status traces for beginning and finishing of instance-level init/term.
    ...
    // Registration events for online defragmentation. Introduced in Windows 10.
    JET_tracetagOLDWork,
    // Work events for online defragmentation progress. Introduced in Windows 10.
    JET_tracetagSysInitTerm,
    // High level status traces for beginning and finishing of one-time system init/term operations during DLL load/unload. Contrast to JET_tracetagInitTerm. Introduced in Windows 10.
    JET_tracetagVersionAndStagingChecks,
    JET_tracetagFile,
    JET_tracetagFlushFileBuffers,
    // Traces for flush file buffers (including suppressed FFB calls).
    ...
    JET_tracetagMax,
    //  Maximum trace tag value (invalid).
} JET_TRACETAG;
...
typedef enum
{
    ...
    //  enable/disable tracing ("ul" param cast to long)
    JET_traceopSetTag,
    //  enable/disable tracing for specified tag ("ul" param cast to long)
    JET_traceopSetAllTags,
    //  enable/disable tracing for all tags ("ul" param cast to long)
    JET_traceopSetMessagePrefix,
    //  text which should prefix all emitted messages ("ul" param cast to char*)
    JET_traceopRegisterTag,
    //  callback to register a trace tag ("ul" param cast to JET_PFNTRACEREGISTER)
    JET_traceopRegisterAllTags,
    //  callback to register all trace tags ("ul" param cast to JET_PFNTRACEREGISTER)
    JET_traceopSetEmitCallback,
    //  override default trace emit function with specified function, or pass NULL to revert to default ("ul" param ast to JET_PFNTRACEEMIT)
    JET_traceopSetThreadidFilter,
    //  threadid to use to filter traces (0==all threads, -1==no threads)
    JET_traceopSetDbidFilter,
    //  JET_DBID to use to filter traces (0x7fffffff==all db's, JET_dbidNil==no db's)
    JET_traceopMax
} JET_TRACEOP;
...
typedef struct tagJET_INDEXCREATEOLD_A
{
    ...
    union
    {
        ...
        // maximum length of variable length columns in index key (if JET_bitIndexTupleLimits not specified)
...
typedef struct
{
    ...
    //  the date of the last time that incremental reseed was initiated on this database
    uint32_t ulIncrementalReseedCountOld;
    //  number of times incremental reseed was initiated on this database before the last defrag
...
struct JET_THREADSTATS4
{
    ...
    uint32_t cSumDatabaseReadQueueDepthImpact;
    //  sum of the impact on disk queue depth made by each database read from disk
    uint32_t cSumDatabaseReadQueueDepth;
    //  sum of the actual disk queue depths experienced by each database read from disk
...
typedef enum
{
    ...
    //  sorted tables
    JET_residVERBUCKET,
    //  buckets used by Vesrion store
    JET_residPAGE,
    //  [deprecated] general purposes page usage (instanceless)
    JET_residSPLIT,
    //  split struct used by BTree page spilt (instanceless)
    JET_residSPLITPATH,
    //  another split struct used by BTree page split (instanceless)
    JET_residMERGE,
    //  merge sruct used by BTree pages merge (instanceless)
    JET_residMERGEPATH,
    //  another merge struct used by BTree page merge (instanceless)
    JET_residINST,
    //  instances (instanceless)
    JET_residLOG,
    //  log instances (instanceless)
    ...
    JET_residKEY,
    //  key buffers (instanceless)
    JET_residBOOKMARK,
    //  bookmark buffers (instanceless)
    JET_residLRUKHIST,
    //  LRUK history records (instanceless)
    JET_residRBSBuf,
    //  Revert snapshot buffers
    JET_residTest,
    //  internal use only: for testing the resource manager
    JET_residMax,
...
typedef enum
{
    JET_resoperNull,
    JET_resoperTag,
    //  Each object class has (unique) tag, get only
    JET_resoperSize,
    //  The size of the object, get only
    JET_resoperAlign,
    //  Alignmet of the object, Set before jetinit
    JET_resoperMinUse,
    //  Min number of preallocated objects, Set before jetinit
    JET_resoperMaxUse,
    //  Max number of allocated object, After jetinit is set per instance
    ...
    //  Get only, with nonnull instance param the count is object based
    ...
    //  offset where to store the pointer to next free object
    //  set before jet init only
    //  NOTE: Debug and JET dev only
    JET_resoperGuard,
    //  Turn the guard page at the end of the section on and off
    //  Set before jetinit only
    JET_resoperAllocTopDown,
    //  chunks are allocated using MEM_TOP_DOWN
    JET_resoperPreserveFreed,
    //  freed chunks are not decommitted/released back to the OS
    JET_resoperAllocFromHeap,
    //  objects are allocated directly from the heap, bypassing resource management
    JET_resoperEnableMaxUse,
    //  Enables the enforcement of the max number of allocated objects
    JET_resoperRCIList,
    //  Expose m_pRCIList for debug extension
    JET_resoperSectionHeader,
    //  Expose cbSectionHeader for debug extension
    JET_resoperMax
} JET_RESOPER;
...
{
    JET_errcatUnknown = 0,
    //      unknown, error retrieving err category
    JET_errcatError,
    //      top level (no errors should be of this class)
    JET_errcatOperation,
    JET_errcatFatal,
    JET_errcatIO,
    //      bad IO issues, may or may not be transient.
    JET_errcatResource,
    JET_errcatMemory,
    //      out of memory (all variants)
    JET_errcatQuota,
    JET_errcatDisk,
    //      out of disk space (all variants)
    JET_errcatData,
    JET_errcatCorruption,
...
typedef struct
{
    ...
    // The time we started reverting from. We will skip adding reverting to time as the caller already gets that info as part of prepare call.
...
typedef struct
{
    ...
    //  user data associated with the record but stored in the long-value tree (NOTE: does NOT count intrinsic long-values)
    ...
    //  total number of values stored in the long-value tree for this record (NOTE: does NOT count intrinsic long-values)
...
typedef struct
{
    ...
    //  compressed size of user data in record (same as cbData if no intrinsic long-values are compressed)
    uint64_t cbLongValueDataCompressed;
    // compressed size of user data in the long-value tree (same as cbLongValue data if no separated long values are compressed)
} JET_RECSIZE2;
typedef struct
{
    uint64_t cbData; //  user data in record
    ...
    uint64_t cbIntrinsicLongValueDataCompressed;
    // compressed size of user data stored in intrinsic LVs (in the record).
...
typedef enum
{
    ...
    //  takes a JET_TESTHOOKTESTINJECTION*
    opTestHookHookNtQueryInformationProcess,
    //  takes a JET_TESTHOOKAPIHOOKING*
    ...
    //  takes a JET_TESTHOOKUNITTEST2
    opTestHookEnforceContextFail,
    //  takes a JET_TESTHOOKAPIHOOKING*, replaces g_pfnEnforceContextFail with user-supplied callback
    opTestHookGetBFLowMemoryCallback,
    //  return pointer to function
    opTestHookTraceTestMarker,
    //  traces a test marker ETW/OSTrace event
    opTestHookGetCheckpointDepth,
    //  return value of the current checkpoint depth
    opTestHookGetOLD2Status,
    //  return whether we are processing B+ tree defrag tasks in the defrag manager
    opTestHookGetEngineTickNow,
    //  returns the TickOSTimeCurrent() from inside the engine
    opTestHookSetEngineTickTime,
    //  takes a JET_TESTHOOKTIMEINJECTION structure
    opTestHookCacheQuery,
    //  takes a JET_TESTHOOKCACHEQUERY*
    opTestHookEvictCache,
    //  takes a JET_TESTHOOKEVICTCACHE*
    opTestHookCorrupt,
    //  takes a JET_TESTHOOKCORRUPT*
    opTestHookEnableAutoIncDeprecated,
    //  REMOVED - takes a ulong*
    opTestHookSetErrorTrap,
    //  takes a ulong*, set g_errTrap
    opTestHookGetTablePgnoFDP,
    //  takes a JET_TABLEID in, returns a pgno
    opTestHookAlterDatabaseFileHeader,
    //  takes a JET_TESTHOOKALTERDBFILEHDR.
    opTestHookGetLogTip,
    //  returns the current log tip LGPOS as a 64 bit integer
    opTestHookBlockCacheTestEnabled,
    //  takes a long* indicating if the block cache should be forced on for test purposes
} TESTHOOK_OP;
...
enum
{
    ...
    // Injecting ERROR_IO_DEVICE (results in Jet_errDiskIO)
    fInvalidAPIUsage = 0x00000400,
    // invalid usage of the JET API
    fInvalidUsage = 0x00000800,
    // invalid usage of some sub-component during unit testing
    fCorruptingPageLogically = 0x00001000,
    // but checksum (and perhaps structure / consistency) is ok.
    fOutOfMemory = 0x00002000,
    fLeakingUnflushedIos = 0x00004000,
    // for internal tests that do not flush file buffers before deleting the pfapi.
    fHangingIOs = 0x00008000,
    // Simulate hung IOs
    fCorruptingWithLostFlushWithinReqRange = 0x00010000,
    // Used to disable specific asserts if lost flushes are within required range.
    fStrictIoPerfTesting = 0x00020000,
    // Use this to turn off any fault injection or test randomization that causes unnecessary / repairative IOs or adverse IO performance effects.
    fDisableAssertReqRangeConsistentLgpos = 0x00040000,
    // Use this to turn off asserts related to setting lgposOB0 or lgposModify below min required.
};
...
#pragma pack(pop)
        ...
        struct // CorruptDatabasePageImage
        {
            ...
            int64_t pgnoTarget;
            //  Page number target (note: this may not seem like it should be required, but it is b/c 4 KB pages xor this into the checksum)
            int64_t iSubTarget; //  Depends upon the JET_bitTestHookCorruptPage* type.
